...
const Post: React.FC<PostProps> = (initialProps) => {
  ...
  useEffect(() => {
    if ((type === "repost" || type === "quote") && parent) {
      api
  ...
  if (type === "repost" && parentData) {
    return (
      <div className={`flex flex-col ${hideInteractive ? "" : "border-b"}`}>
        ...
        <Post key={parentData.id} {...parentData} />
      </div>
    );
  }
      ...
      <div className={`flex p-4 ${hideInteractive ? "" : "border-b"}`}>
        ...
        <div className="flex-1">
          ...
          <div className="mt-2">
            ...
            {/* quote */}
            {initialProps.type === "quote" && parentData && (
          ...
          {!hideInteractive && (
            <div className="flex justify-center gap-40 mt-3 text-gray-600 text-sm">
              <div
                onClick={() => setShowCompose({ mode: "reply", parentId: id })}
              ...
              <div className="flex items-center space-x-1">
                <RepostMenu post={post} onQuote={handleQuote} />
              </div>
              <div
                onClick={toggle}
                className="cursor-pointer flex items-center space-x-1"
